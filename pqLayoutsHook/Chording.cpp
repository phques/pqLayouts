...
void Chording::Init(const ChordingKeys& _stenoKeys)
{
...
const ChordingKey* Chording::GetChordingKeyFromQwerty(VeeKee qwertyVk) const
{
...
const ChordingKey* Chording::GetChordingKeyFromSteno(VeeKee stenoVk) const
{
...
bool Chording::IsConsonant(VeeKee vk) const
{
    return consonants.find(vk) != consonants.end();
}

bool Chording::IsHandDelimiterKey(VeeKee vk) const
{
    return handDelimiters.find(vk) != handDelimiters.end();
}

std::string Chording::ToString(const Kord& kord) const
{
...
PQHOOK_API VeeKee Chording::AdjustForRightHand(VeeKee vk) const
{
    return IsConsonant(vk) ? vk | RIGHTHANDFLAG : vk;
}

// make right hand consonants unique
PQHOOK_API VeeKee Chording::AdjustForHandedness(VeeKee vk, bool rightHand) const
{
